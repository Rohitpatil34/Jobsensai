...
const analyzeResumeAgainstCategory = async (resumeText, category, userId) => {
  ...
  const prompt = `You are an expert resume evaluator. Analyze the following resume text and determine how suitable the candidate is for a role in the category: "${category}".

Do not evaluate based on a specific job description. Instead, identify the most appropriate job role that fits the candidate's skills, experience, and strengths as reflected in the resume.

Return a clean JSON in this format:
{
  ...
  "recommendedjob": "most suitable role based on resume give only one",
  "SuggestedProject":"Suggest project to enhance my resume in array suggest multiple projects",
  "SuggestedCertification":"Suggest certificates to do in array suugest multiple certificates"
}
